...
function Player(x, y, context, maxX, maxY){
    ...
    this.bounder = new Bounder("rect", new Rect(this.position.x, this.position.y + 29, s.width, s.height - 37));
};
...
Player.prototype.update = function Player_update(inputHandler, deltaTime){
    ...
    if (inputHandler.checkKey(40) && this.position.y + s.height + this.speed <= this.maxY) { this.position.y += this.speed; }
    if (inputHandler.checkKey(37) && this.position.x >= this.speed) { this.position.x -= this.speed; }
    if (inputHandler.checkKey(39) && this.position.x + s.width + this.speed <= this.maxX) { this.position.x += this.speed; }
